...
class CircularLinkedList {
  ...
  Node * nodeExists(int k) {
    ...
    if (ptr == NULL) {
      return temp;
        ...
        // Here we are using do while loop because for the first time ptr is equal to head
        // so if we used while loops, this loop will never be executed.
      do {
        if (ptr -> key == k) {
          temp = ptr;
        }
  ...
  void appendNode(Node * new_node) {
    if (nodeExists(new_node -> key) != NULL) {
      ...
      if (head == NULL) {
            //here even if we have only one node in the list , that node should also point back to itself
        head = new_node;
        new_node -> next = head;
